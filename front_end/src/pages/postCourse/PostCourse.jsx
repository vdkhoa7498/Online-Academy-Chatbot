...
import { Form, Input, Button, Upload, message, Divider, Select, Modal, InputNumber, Spin } from 'antd';
...
const PostCourse = ({ categories }) => {
    ...
    const onFinish = async (values) => {
        ...
        await httpClient.course.createCourse(course)
            .then(async res => {
                const videoForm = {
                    courseId: res.id,
                    videoList: videos
                }
                await httpClient.video.createVideos(videoForm)
                    .then(res => {
    ...
    return (
        <Spin spinning={isPageLoading} tip="Loading..." size="large" style={{textAlign: 'center'}}>
                                ...
                                rules={[
                                    () => ({
                                        validator() {
                                            ...
                                            return Promise.reject('Vui lòng thêm một hình đại diện khoá học!');
                                        },
                                    }),
                                ...
                                >
                                    {(fields, { add, remove }) => (
                                        <>
                                            {fields.map(field => (
                                                <div key={field.key}
                                                    style={{ width: "100%", display: "flex", alignItems: "start", justifyContent: "start", flexDirection: "column" }} align="baseline">
                                                    <Form.Item
                                                        noStyle
                                                    ...
                                                    <Button onClick={() => remove(field.name)} style={{ marginLeft: 5, marginBottom: 10, color: 'red' }} icon={<MinusCircleOutlined />}>
                                                        Remove video
                                                    </Button>
                                                    <Divider />
                                                </div>
